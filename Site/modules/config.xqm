...
(:~
 ...
 :)
declare variable $config:default-language := "fr";
...
declare variable $config:facets := [
    ...
    map {
        "dimension": "year",
        "heading": "Années de publication",
        "max": 5,
        "hierarchical": true(),
        "output": function($label) {
            switch($label)
                case "Sans Date" return "Sans date"
                default return $label
        }
    },
    map {
        ...
        "output": function($label) {
            switch($label)
                ...
                case "vers/prose" return "Vers et prose"
                default return $label
        }
    },
    map {
    ...
    map {
        "dimension": "privilege",
        "heading": "Privilège d'impression",
        "max": 2,
        "hierarchical": true(),
        "output": function($label) {
            switch($label)
    ...
    map {
        "dimension": "handwritten",
        "heading": "Annotation manuscrite",
        "max": 2,
        "hierarchical": false(),
        "output": function($label) {
            switch($label)
...
(: 
 : Variable utile à la recherche dans le corpus, et non pas dans l'ensemble des documents XML :)
declare variable $config:data-corpus :=$config:app-root || "/data/corpus";
...
(:~
 ...
 :)
declare variable $config:default-odd :="Antonomaz.odd";
...
(:~
 ...
 :)
declare variable $config:odd-available :=("Antonomaz.odd", "printers.odd", "notices.odd");
...
declare function config:collection-config($collection as xs:string?, $docUri as xs:string?) {
    ...
    switch ($collection)
        ...
        case "imprimeurs" return
            map {
                "odd": "printers.odd",
                "view": "body",
                "depth": $config:pagination-depth,
                "fill": $config:pagination-fill,
                "template": "view-printers.html"
            }
        case "notices/institution" return
            map {
                "odd": "notices.odd",
                "view": "body",
                "depth": $config:pagination-depth,
                "fill": $config:pagination-fill,
                "template": "view-notices.html"
            }
        case "notices/histoire" return
            map {
                "odd": "notices.odd",
                "view": "body",
                "depth": $config:pagination-depth,
        ...
        case "notices/litterature" return
            map {
                "odd": "notices.odd",
                "view": "body",
                "depth": $config:pagination-depth,
