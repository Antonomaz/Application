...
(:  modification des fonctions pour que l'élément racine teiCorpus soit aussi pris en compte :)
declare function nav:get-root($root as xs:string?, $options as map(*)?) {
    $config:data-default ! (
        for $doc in collection(. || "/" || $root)//tei:text[ft:query(., "file:*", $options)]
        let $teiCorpus := ($doc/ancestor::tei:teiCorpus)
        return
            if ($teiCorpus) then
                $doc/ancestor::tei:teiCorpus
            else
...
declare function nav:get-header($config as map(*), $node as element()) {
    let $teiCorpus := ($node/self::tei:teiCorpus)
    return
            if ($teiCorpus) then
...
declare function nav:get-metadata($config as map(*), $root as element(), $field as xs:string) {
    switch ($field)
        case "title_teiCorpus" return
            (
                $root/tei:teiHeader//tei:titleStmt/tei:title/tei:surname
            )
        case "commentaire" return
            (
                $root/tei:teiHeader//tei:abstract/p[@source="Mazarine"],
                $root/tei:teiHeader//tei:abstract/p[@source="Antonomaz"]
            )
        case "printers" return
            (
                $root/tei:teiHeader/tei:fileDesc/tei:titleStmt/tei:title/tei:surname/string()
            )
        case "title" return
        ...
        case "subject" return
            ($root//tei:keywords/tei:term[@type="subject"])
        ...
        case "genre_rech" return
            ($root//tei:keywords//tei:term[@type="genre"])
