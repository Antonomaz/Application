            ...
            switch ($field)
                case "title" return
                    if (exists($target-texts)) then
                        ...
                        return
                            $config:data-corpus ! doc(. || "/" || $text)//tei:title[ft:query(., $query, query:options($sortBy))]
                    else
                        collection($config:data-corpus)//tei:title[ft:query(., $query, query:options($sortBy))]
                case "commentaire" return
                    if (exists($target-texts)) then
                        for $text in $target-texts
                        return
                            $config:data-corpus ! doc(. || "/" || $text)//tei:abstract[ft:query(., $query, query:options($sortBy))]
                    else
                        collection($config:data-corpus)//tei:abstract[ft:query(., $query, query:options($sortBy))]
                case "corpus" return
                    if (exists($target-texts)) then
                        for $text in $target-texts
                        return
                            $config:data-corpus ! doc(. || "/" || $text)//tei:text[ft:query(., $query, query:options($sortBy))]
                    else
                        collection($config:data-corpus)//tei:text[ft:query(., $query, query:options($sortBy))]
                default return
                    if (exists($target-texts)) then
                        for $text in $target-texts
                        return
                            $config:data-root ! doc(. || "/" || $text)//tei:div[ft:query(., $query, query:options($sortBy))] |
                            $config:data-root ! doc(. || "/" || $text)//tei:abstract[ft:query(., $query, query:options($sortBy))] |
                            $config:data-root ! doc(. || "/" || $text)//tei:title[ft:query(., $query, query:options($sortBy))] |
                            $config:data-root ! doc(. || "/" || $text)//tei:text[ft:query(., $query, query:options($sortBy))] |
                            $config:data-root ! doc(. || "/" || $text)//tei:p[ft:query(., $query, query:options($sortBy))]
                    else
                        collection($config:data-root)//tei:div[ft:query(., $query, query:options($sortBy))] |
                        collection($config:data-root)//tei:abstract[ft:query(., $query, query:options($sortBy))] |
                        collection($config:data-root)//tei:title[ft:query(., $query, query:options($sortBy))] |
    ...
    return
        switch ($field)
            ...
            case "printers" return
                collection($config:data-root)/ft:index-keys-for-field("printers", $lower-case-q,
                    function($key, $count) {
                        $key
                    }, 30)
            case "commentaire" return
                collection($config:data-root)/ft:index-keys-for-field("commentaire", $lower-case-q,
                    function($key, $count) {
                        $key
                    }, 30)
            case "publisher_rech" return
                collection($config:data-root)/ft:index-keys-for-field("publisher_rech", $lower-case-q,
                    function($key, $count) {
                        $key
                    }, 30)
            case "subject" return
                collection($config:data-root)/ft:index-keys-for-field("subject", $lower-case-q,
                    function($key, $count) {
                        $key
                    }, 30)
            case "genre_rech" return
                collection($config:data-root)/ft:index-keys-for-field("genre_rech", $lower-case-q,
                    function($key, $count) {
                        $key
                    }, 30)
            case "date_rech" return
                collection($config:data-root)/ft:index-keys-for-field("date_rech", $lower-case-q,
                    function($key, $count) {
                        $key
                    }, 30)
            case "identifier" return
                collection($config:data-root)/ft:index-keys-for-field("identifier", $lower-case-q,
                    function($key, $count) {
                        $key
                    }, 30)
            case "author" return
    ...
    return
        switch ($field)
            case "abstract" return
                $context[./descendant-or-self::tei:abstract[ft:query(., $query, $query:QUERY_OPTIONS)]]
            default return
