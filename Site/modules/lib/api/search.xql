    ...
    return
        <paper-card>
            ...
            <div class="matches">
            {
                ...
                (:let $config := <config width="60" table="no" link="{$docId}?{if ($docLink) then 'root=' || $docLink || '&amp;' else ()}action=search&amp;template=view-text.html&amp;odd={$config?odd}#{$matchId}"/>:)
                let $config := <config width="60" table="no" link="{$docId}?{if ($docLink) then 'root=' || $docLink || '&amp;' else ()}action=search&amp;view={$config?view}&amp;odd={$config?odd}#{$matchId}" target="_blank"/>
                return
