    ...
    return
        switch ($field)
            ...
            case "publisher_rech" return
                collection($config:data-root)/ft:index-keys-for-field("publisher_rech", $q,
                    function($key, $count) {
                        $key
                    }, 30)
            case "subject" return
                collection($config:data-root)/ft:index-keys-for-field("subject", $q,
                    function($key, $count) {
                        $key
                    }, 30)
            case "genre_rech" return
                collection($config:data-root)/ft:index-keys-for-field("genre_rech", $q,
                    function($key, $count) {
                        $key
                    }, 30)
            case "date_rech" return
                collection($config:data-root)/ft:index-keys-for-field("date_rech", $q,
                    function($key, $count) {
                        $key
                    }, 30)
            case "identifier" return
                collection($config:data-root)/ft:index-keys-for-field("identifier", $q,
                    function($key, $count) {
                        $key
                    }, 30)
            case "author" return
